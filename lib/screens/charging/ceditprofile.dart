...
import 'package:newpro/screens/charging/cprofile.dart';
...
class ChargeEditProfile extends StatefulWidget {
  String _id;
  String Log_id;

  ChargeEditProfile(this._id, this.Log_id);

  @override
  State<ChargeEditProfile> createState() => _ChargeEditProfileState();
}

class _ChargeEditProfileState extends State<ChargeEditProfile> {
...
/*  Future<void> getLogin() async {
  ...
  Future<void> _viewBattery() async {
    ...
    print('Log Id ${_id}');
    var res =
        await Api().getData('/api/user/view-charging-station-profile/' + _id);
  ...
  _update() async {
    ...
    print(data);
    var res =
        await Api().authData(data, '/api/user/update-charging-station/' + _id);
    var body = json.decode(res.body);
    print(res);
    ...
    if (body['success'] == true) {
      ...
      Navigator.push(
          context, MaterialPageRoute(builder: (context) => ChargeProfile()));
    } else {
      Fluttertoast.showToast(
        msg: body['message'].toString(),
        backgroundColor: Colors.grey,
